...
#Esto indicará a Docker que cree (build) la imagen del Dockerfile a nivel del root ( . ) y le ponga la etiqueta "-t" como "node-example". 
#No olvide el punto, ya que así es como Docker sabe dónde buscar el Dockerfile.
#docker run -p 3141:8080 -d node-example
#Esto indicará a Docker que ejecute (run) la imagen creada y etiquetada como "user1/node-example", que exponga el puerto 3141 en el equipo que haga de host, 
#que busque el puerto 8080 en el contendedor de Docker (-p 3141:8080) y que ponga en marcha el proceso como si fuera un proceso daemon (-d) 
